...
@Component({
  selector: 'wm-receives-other-edit',
  templateUrl: './receives-other-edit.component.html',
  styles: []
})
export class ReceivesOtherEditComponent implements OnInit {
  ...
  addProduct() {
    ...
    if (idx > -1) {
      ...
      this.wareHouseService.detail(product.warehouse_id)
        .then((result: any) => {
          product.warehouse_name = result.detail.warehouse_name;
        })
      ...
      if (this.selectedGenericId && this.selectedProductId && this.selectedReceiveQty) {
    ...
    try {
      ...
      if (rs.ok) {
        rs.rows.forEach(v => {
          ...
          this.wareHouseService.detail(v.warehouse_id)
            .then((result: any) => {
              product.warehouse_name = result.detail.warehouse_name;
            })
