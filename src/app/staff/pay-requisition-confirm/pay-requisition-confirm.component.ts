...
import { PayRequisitionService } from 'app/staff/pay-requisition.service';
...
@Component({
  selector: 'wm-pay-requisition-confirm',
  templateUrl: './pay-requisition-confirm.component.html',
  styles: []
})
export class PayRequisitionConfirmComponent implements OnInit {
  ...
  async getOrderDetail() {
    ...
    try {
      ...
      this.modalLoading.hide();
      if (rs.ok) {
        const detail: IRequisitionOrder = <IRequisitionOrder>rs.rows;
  ...
  async saveWithOutUnPaid(data: any) {
    ...
    try {
      ...
      if (rs.ok) {
        ...
        this.router.navigate(['/staff/pay-requisition']);
      } else {
        this.alertService.error(rs.error);
      }
    } catch (error) {
