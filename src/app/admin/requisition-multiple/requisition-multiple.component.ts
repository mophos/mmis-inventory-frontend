...
@Component({
  selector: 'wm-requisition-multiple',
  templateUrl: './requisition-multiple.component.html',
  styles: []
})
export class RequisitionMultipleComponent implements OnInit {
  ...
  // }
  addWarehouse() {
    const idxR = _.findIndex(this.warehouses, { 'warehouse_id': +this.wmRequisition });
    const idxW = _.findIndex(this.withDrawWarehouses, { 'destination_warehouse_id': +this.wmWithdraw });
    if (idxR > -1 && idxW > -1) {
      const obj = {
        wmRequisitionId: this.wmRequisition,
        wmRequisitionName: this.warehouses[idxR].warehouse_name,
        wmWithdrawId: this.wmWithdraw,
        wmWithdrawName: this.withDrawWarehouses[idxW].warehouse_name
      }
  ...
  removeWarehouse(warehouseId) {
    const idx = _.findIndex(this.warehouseList, { 'warehouse_id': +warehouseId });
    if (idx > -1) {
      this.warehouseList.splice(idx, 1);
    }
  }
}
